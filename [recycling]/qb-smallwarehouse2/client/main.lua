...
local entranceTargetID = 'entranceSmallTwoTarget'
local isInsideEntranceZone = false
local entranceZone = nil

local exitTargetID = 'exitSmallTwoITarget'
...
local function DestroyPickupTarget()
  ...
  if Config.UseTarget then
	...
	--Warehouse SmallTwo
local function RegisterEntranceSmallTwoTarget()
  local coords = vector3(Config.SmallTwoOutsideLocation.x, Config.SmallTwoOutsideLocation.y, Config.SmallTwoOutsideLocation.z)

  if Config.UseTarget then
    entranceZone = exports['qb-target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
      name = entranceTargetID,
      heading = 44.0,
      minZ = Config.SmallTwoOutsideLocation.z - 1.0,
      maxZ = Config.SmallTwoOutsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-SmallTwo:client:target:enterLocation',
...
local function RegisterExitTarget()
  local coords = vector3(Config.SmallTwoInsideLocation.x, Config.SmallTwoInsideLocation.y, Config.SmallTwoInsideLocation.z)

  if Config.UseTarget then
    exitZone = exports['qb-target']:AddBoxZone(exitTargetID, coords, 1, 4, {
      name = exitTargetID,
      heading = 270,
      minZ = Config.SmallTwoInsideLocation.z - 1.0,
      maxZ = Config.SmallTwoInsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-SmallTwo:client:target:exitLocation',
...
local function RegisterDutyTarget()
  local coords = vector3(Config.SmallTwoDutyLocation.x, Config.SmallTwoDutyLocation.y, Config.SmallTwoDutyLocation.z)

  if Config.UseTarget then
    dutyZone = exports['qb-target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
      name = dutyTargetID,
      heading = 270,
      minZ = Config.SmallTwoDutyLocation.z - 2.0,
      maxZ = Config.SmallTwoDutyLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-SmallTwo:client:target:toggleDuty',
...
local function RegisterDeliveyTarget()
  local coords = vector3(Config.SmallTwoDropLocation.x, Config.SmallTwoDropLocation.y, Config.SmallTwoDropLocation.z)

  if Config.UseTarget then
    deliveryZone = exports['qb-target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
      name = deliveryTargetID,
      heading = 270,
      minZ = Config.SmallTwoDropLocation.z - 2.0,
      maxZ = Config.SmallTwoDropLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-SmallTwo:client:target:dropPackage',
...
local function GetRandomPackage()
  packageCoords = Config.SmallTwoPickupLocations[math.random(1, #Config.SmallTwoPickupLocations)]
...
local function SetLocationBlip()
  local RecycleBlip = AddBlipForCoord(Config.SmallTwoOutsideLocation.x, Config.SmallTwoOutsideLocation.y, Config.SmallTwoOutsideLocation.z)
  ...
  AddTextComponentSubstringPlayerName('Small Recycling Warehouse')
  EndTextCommandSetBlipName(RecycleBlip)
end

local function buildInteriorDesign()
  for _, pickuploc in pairs(Config.SmallTwoPickupLocations) do
    local model = GetHashKey(Config.SmallTwoWarehouseObjects[math.random(1, #Config.SmallTwoWarehouseObjects)])
    RequestModel(model)
...
local function EnterLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.SmallTwoInsideLocation.x, Config.SmallTwoInsideLocation.y, Config.SmallTwoInsideLocation.z)
  buildInteriorDesign()
  DoScreenFadeIn(500)
...
local function ExitLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.SmallTwoOutsideLocation.x, Config.SmallTwoOutsideLocation.y, Config.SmallTwoOutsideLocation.z + 1)
  DoScreenFadeIn(500)
  ...
  if Config.UseTarget then
    pickupZone = exports['qb-target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
      ...
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-SmallTwo:client:target:pickupPackage',
...
RegisterNetEvent('qb-recyclejob-SmallTwo:client:target:enterLocation', function()
  EnterLocation()
end)

RegisterNetEvent('qb-recyclejob-SmallTwo:client:target:exitLocation', function()
  ExitLocation()
end)

RegisterNetEvent('qb-recyclejob-SmallTwo:client:target:toggleDuty', function()
  onDuty = not onDuty
  if onDuty then
...
RegisterNetEvent('qb-recyclejob-SmallTwo:client:target:pickupPackage', function()
...
RegisterNetEvent('qb-recyclejob-SmallTwo:client:target:dropPackage', function()
  ...
  QBCore.Functions.Progressbar('deliver_reycle_package',  Lang:t("text.unpacking_the_package"), Config.DeliveryActionDuration, false, true, {
      ...
      -- Done
      StopAnimTask(PlayerPedId(), 'mp_car_bomb', 'car_bomb_mechanic', 1.0)
      TriggerServerEvent('qb-smallwarehousetwo:server:getItemOneSixtyNine')
      GetRandomPackage()
  end)
end)
...
CreateThread(function()
  ...
  SetLocationBlip()
  RegisterEntranceSmallTwoTarget()
    ...
    while true do
      ...
      if isInsideEntranceZone then
        ...
        if IsControlJustReleased(0, 38) then
          ...
          Wait(500)
          TriggerEvent('qb-recyclejob-SmallTwo:client:target:SmallTwoenterLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideExitZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-SmallTwo:client:target:SmallTwoexitLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideDutyZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-SmallTwo:client:target:toggleDuty')
          exports['qb-core']:HideText()
        end
      end
      ...
      if onDuty then
        if isInsidePickupZone and not carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-SmallTwo:client:target:pickupPackage')
            exports['qb-core']:HideText()
          end
        ...
        if isInsideDeliveryZone and carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-SmallTwo:client:target:dropPackage')
            exports['qb-core']:HideText()
          end
        end
      end
