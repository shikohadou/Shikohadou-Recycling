...
local entranceTargetID = 'entranceSmallCarTarget'
local isInsideEntranceZone = false
local entranceZone = nil

local exitTargetID = 'exitSmallCarITarget'
...
local function DestroyPickupTarget()
  ...
  if Config.UseTarget then
	...
	--Warehouse SmallCar
local function RegisterEntranceSmallCarTarget()
  local coords = vector3(Config.SmallCarOutsideLocation.x, Config.SmallCarOutsideLocation.y, Config.SmallCarOutsideLocation.z)

  if Config.UseTarget then
    entranceZone = exports['qb-target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
      name = entranceTargetID,
      heading = 44.0,
      minZ = Config.SmallCarOutsideLocation.z - 1.0,
      maxZ = Config.SmallCarOutsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-SmallCar:client:target:enterLocation',
          label = Lang:t("text.enter_SmallCar_warehouse"),
        },
      },
      distance = 1.0
    })
    ...
    entranceZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText(Lang:t("text.point_enter_SmallCar_warehouse"), 'left')
      else
...
local function RegisterExitTarget()
  local coords = vector3(Config.SmallCarInsideLocation.x, Config.SmallCarInsideLocation.y, Config.SmallCarInsideLocation.z)

  if Config.UseTarget then
    exitZone = exports['qb-target']:AddBoxZone(exitTargetID, coords, 1, 4, {
      name = exitTargetID,
      heading = 270,
      minZ = Config.SmallCarInsideLocation.z - 1.0,
      maxZ = Config.SmallCarInsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-SmallCar:client:target:exitLocation',
          label = Lang:t("text.exit_SmallCar_warehouse"),
        },
      },
      distance = 1.0
    })
    ...
    exitZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText(Lang:t("text.point_exit_SmallCar_warehouse"), 'left')
      else
        exports['qb-core']:HideText()
      end
...
local function RegisterDutyTarget()
  local coords = vector3(Config.SmallCarDutyLocation.x, Config.SmallCarDutyLocation.y, Config.SmallCarDutyLocation.z)

  if Config.UseTarget then
    dutyZone = exports['qb-target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
      name = dutyTargetID,
      heading = 270,
      minZ = Config.SmallCarDutyLocation.z - 2.0,
      maxZ = Config.SmallCarDutyLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-SmallCar:client:target:toggleDuty',
...
local function RegisterDeliveyTarget()
  local coords = vector3(Config.SmallCarDropLocation.x, Config.SmallCarDropLocation.y, Config.SmallCarDropLocation.z)

  if Config.UseTarget then
    deliveryZone = exports['qb-target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
      name = deliveryTargetID,
      heading = 270,
      minZ = Config.SmallCarDropLocation.z - 2.0,
      maxZ = Config.SmallCarDropLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-SmallCar:client:target:dropPackage',
...
local function GetRandomPackage()
  packageCoords = Config.SmallCarPickupLocations[math.random(1, #Config.SmallCarPickupLocations)]
...
local function DropPackage()
  ...
  carryPackage = nil
	...
	--Warehouse SmallCar Blip
local function SetLocationBlip()
  local RecycleBlip = AddBlipForCoord(Config.SmallCarOutsideLocation.x, Config.SmallCarOutsideLocation.y, Config.SmallCarOutsideLocation.z)
  ...
  AddTextComponentSubstringPlayerName('Small Car Recycling Warehouse')
  EndTextCommandSetBlipName(RecycleBlip)
end

local function buildInteriorDesign()
  for _, pickuploc in pairs(Config.SmallCarPickupLocations) do
    local model = GetHashKey(Config.SmallCarWarehouseObjects[math.random(1, #Config.SmallCarWarehouseObjects)])
    RequestModel(model)
...
local function EnterLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.SmallCarInsideLocation.x, Config.SmallCarInsideLocation.y, Config.SmallCarInsideLocation.z)
  buildInteriorDesign()
  DoScreenFadeIn(500)
...
local function ExitLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.SmallCarOutsideLocation.x, Config.SmallCarOutsideLocation.y, Config.SmallCarOutsideLocation.z + 1)
  DoScreenFadeIn(500)
  ...
  if Config.UseTarget then
    pickupZone = exports['qb-target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
      ...
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-SmallCar:client:target:pickupPackage',
...
RegisterNetEvent('qb-recyclejob-SmallCar:client:target:enterLocation', function()
  EnterLocation()
end)

RegisterNetEvent('qb-recyclejob-SmallCar:client:target:exitLocation', function()
  ExitLocation()
end)

RegisterNetEvent('qb-recyclejob-SmallCar:client:target:toggleDuty', function()
  onDuty = not onDuty
  if onDuty then
...
RegisterNetEvent('qb-recyclejob-SmallCar:client:target:pickupPackage', function()
...
RegisterNetEvent('qb-recyclejob-SmallCar:client:target:dropPackage', function()
  ...
  QBCore.Functions.Progressbar('deliver_reycle_package',  Lang:t("text.unpacking_the_package"), Config.DeliveryActionDuration, false, true, {
      ...
      -- Done
      StopAnimTask(PlayerPedId(), 'mp_car_bomb', 'car_bomb_mechanic', 1.0)
      TriggerServerEvent('qb-smallcarwarehouse:server:getItemOneTwo')
      GetRandomPackage()
  end)
end)
...
CreateThread(function()
  ...
  SetLocationBlip()
  RegisterEntranceSmallCarTarget()
    ...
    while true do
      ...
      if isInsideEntranceZone then
        ...
        if IsControlJustReleased(0, 38) then
          ...
          Wait(500)
          TriggerEvent('qb-recyclejob-SmallCar:client:target:SmallCarenterLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideExitZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-SmallCar:client:target:SmallCarexitLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideDutyZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-SmallCar:client:target:toggleDuty')
          exports['qb-core']:HideText()
        end
      end
      ...
      if onDuty then
        if isInsidePickupZone and not carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-SmallCar:client:target:pickupPackage')
            exports['qb-core']:HideText()
          end
        ...
        if isInsideDeliveryZone and carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-SmallCar:client:target:dropPackage')
            exports['qb-core']:HideText()
          end
        end
      end
