...
local entranceTargetID = 'entranceLargeTwoTarget'
local isInsideEntranceZone = false
local entranceZone = nil

local exitTargetID = 'exitLargeTwoITarget'
...
local function DestroyPickupTarget()
  ...
  if Config.UseTarget then
	...
	--Warehouse LargeTwo
local function RegisterEntranceLargeTwoTarget()
  local coords = vector3(Config.LargeTwoOutsideLocation.x, Config.LargeTwoOutsideLocation.y, Config.LargeTwoOutsideLocation.z)

  if Config.UseTarget then
    entranceZone = exports['qb-target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
      name = entranceTargetID,
      heading = 44.0,
      minZ = Config.LargeTwoOutsideLocation.z - 1.0,
      maxZ = Config.LargeTwoOutsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-LargeTwo:client:target:enterLocation',
          label = Lang:t("text.enter_LargeTwo_warehouse"),
        },
      },
      distance = 1.0
    })
    ...
    entranceZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText(Lang:t("text.point_enter_LargeTwo_warehouse"), 'left')
      else
...
local function RegisterExitTarget()
  local coords = vector3(Config.LargeTwoInsideLocation.x, Config.LargeTwoInsideLocation.y, Config.LargeTwoInsideLocation.z)

  if Config.UseTarget then
    exitZone = exports['qb-target']:AddBoxZone(exitTargetID, coords, 1, 4, {
      name = exitTargetID,
      heading = 270,
      minZ = Config.LargeTwoInsideLocation.z - 1.0,
      maxZ = Config.LargeTwoInsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-LargeTwo:client:target:exitLocation',
          label = Lang:t("text.exit_LargeTwo_warehouse"),
        },
      },
      distance = 1.0
    })
    ...
    exitZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText(Lang:t("text.point_exit_LargeTwo_warehouse"), 'left')
      else
        exports['qb-core']:HideText()
      end
...
local function RegisterDutyTarget()
  local coords = vector3(Config.LargeTwoDutyLocation.x, Config.LargeTwoDutyLocation.y, Config.LargeTwoDutyLocation.z)

  if Config.UseTarget then
    dutyZone = exports['qb-target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
      name = dutyTargetID,
      heading = 270,
      minZ = Config.LargeTwoDutyLocation.z - 2.0,
      maxZ = Config.LargeTwoDutyLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-LargeTwo:client:target:toggleDuty',
...
local function RegisterDeliveyTarget()
  local coords = vector3(Config.LargeTwoDropLocation.x, Config.LargeTwoDropLocation.y, Config.LargeTwoDropLocation.z)

  if Config.UseTarget then
    deliveryZone = exports['qb-target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
      name = deliveryTargetID,
      heading = 270,
      minZ = Config.LargeTwoDropLocation.z - 2.0,
      maxZ = Config.LargeTwoDropLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-LargeTwo:client:target:dropPackage',
...
local function GetRandomPackage()
  packageCoords = Config.LargeTwoPickupLocations[math.random(1, #Config.LargeTwoPickupLocations)]
...
local function DropPackage()
  ...
  carryPackage = nil
	...
	--Warehouse LargeTwo Blip
local function SetLocationBlip()
  -- local RecycleBlip = AddBlipForCoord(Config.LargeTwoOutsideLocation.x, Config.LargeTwoOutsideLocation.y, Config.LargeTwoOutsideLocation.z)
...
local function buildInteriorDesign()
  for _, pickuploc in pairs(Config.LargeTwoPickupLocations) do
    local model = GetHashKey(Config.LargeTwoWarehouseObjects[math.random(1, #Config.LargeTwoWarehouseObjects)])
    RequestModel(model)
...
local function EnterLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.LargeTwoInsideLocation.x, Config.LargeTwoInsideLocation.y, Config.LargeTwoInsideLocation.z)
  buildInteriorDesign()
  DoScreenFadeIn(500)
...
local function ExitLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.LargeTwoOutsideLocation.x, Config.LargeTwoOutsideLocation.y, Config.LargeTwoOutsideLocation.z + 1)
  DoScreenFadeIn(500)
  ...
  if Config.UseTarget then
    pickupZone = exports['qb-target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
      ...
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-LargeTwo:client:target:pickupPackage',
...
RegisterNetEvent('qb-recyclejob-LargeTwo:client:target:enterLocation', function()
  EnterLocation()
end)

RegisterNetEvent('qb-recyclejob-LargeTwo:client:target:exitLocation', function()
  ExitLocation()
end)

RegisterNetEvent('qb-recyclejob-LargeTwo:client:target:toggleDuty', function()
  onDuty = not onDuty
  if onDuty then
...
RegisterNetEvent('qb-recyclejob-LargeTwo:client:target:pickupPackage', function()
...
RegisterNetEvent('qb-recyclejob-LargeTwo:client:target:dropPackage', function()
  ...
  QBCore.Functions.Progressbar('deliver_reycle_package',  Lang:t("text.unpacking_the_package"), Config.DeliveryActionDuration, false, true, {
      ...
      -- Done
      StopAnimTask(PlayerPedId(), 'mp_car_bomb', 'car_bomb_mechanic', 1.0)
      TriggerServerEvent('qb-largewarehouse2:server:getItemFoxtrot')
      GetRandomPackage()
  end)
end)
...
CreateThread(function()
  ...
  SetLocationBlip()
  RegisterEntranceLargeTwoTarget()
    ...
    while true do
      ...
      if isInsideEntranceZone then
        ...
        if IsControlJustReleased(0, 38) then
          ...
          Wait(500)
          TriggerEvent('qb-recyclejob-LargeTwo:client:target:LargeTwoenterLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideExitZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-LargeTwo:client:target:LargeTwoexitLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideDutyZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-LargeTwo:client:target:toggleDuty')
          exports['qb-core']:HideText()
        end
      end
      ...
      if onDuty then
        if isInsidePickupZone and not carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-LargeTwo:client:target:pickupPackage')
            exports['qb-core']:HideText()
          end
        ...
        if isInsideDeliveryZone and carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-LargeTwo:client:target:dropPackage')
            exports['qb-core']:HideText()
          end
        end
      end
