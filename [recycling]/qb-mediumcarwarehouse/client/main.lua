...
local entranceTargetID = 'entranceMediumCarTarget'
local isInsideEntranceZone = false
local entranceZone = nil

local exitTargetID = 'exitMediumCarITarget'
...
local function DestroyPickupTarget()
  ...
  if Config.UseTarget then
	...
	--Warehouse MediumCar
local function RegisterEntranceMediumCarTarget()
  local coords = vector3(Config.MediumCarOutsideLocation.x, Config.MediumCarOutsideLocation.y, Config.MediumCarOutsideLocation.z)

  if Config.UseTarget then
    entranceZone = exports['qb-target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
      name = entranceTargetID,
      heading = 44.0,
      minZ = Config.MediumCarOutsideLocation.z - 1.0,
      maxZ = Config.MediumCarOutsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-MediumCar:client:target:enterLocation',
          label = Lang:t("text.enter_MediumCar_warehouse"),
        },
      },
      distance = 1.0
    })
    ...
    entranceZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText(Lang:t("text.point_enter_MediumCar_warehouse"), 'left')
      else
...
local function RegisterExitTarget()
  local coords = vector3(Config.MediumCarInsideLocation.x, Config.MediumCarInsideLocation.y, Config.MediumCarInsideLocation.z)

  if Config.UseTarget then
    exitZone = exports['qb-target']:AddBoxZone(exitTargetID, coords, 1, 4, {
      name = exitTargetID,
      heading = 270,
      minZ = Config.MediumCarInsideLocation.z - 1.0,
      maxZ = Config.MediumCarInsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-MediumCar:client:target:exitLocation',
          label = Lang:t("text.exit_MediumCar_warehouse"),
        },
      },
      distance = 1.0
    })
    ...
    exitZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText(Lang:t("text.point_exit_MediumCar_warehouse"), 'left')
      else
        exports['qb-core']:HideText()
      end
...
local function RegisterDutyTarget()
  local coords = vector3(Config.MediumCarDutyLocation.x, Config.MediumCarDutyLocation.y, Config.MediumCarDutyLocation.z)

  if Config.UseTarget then
    dutyZone = exports['qb-target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
      name = dutyTargetID,
      heading = 270,
      minZ = Config.MediumCarDutyLocation.z - 2.0,
      maxZ = Config.MediumCarDutyLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-MediumCar:client:target:toggleDuty',
...
local function RegisterDeliveyTarget()
  local coords = vector3(Config.MediumCarDropLocation.x, Config.MediumCarDropLocation.y, Config.MediumCarDropLocation.z)

  if Config.UseTarget then
    deliveryZone = exports['qb-target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
      name = deliveryTargetID,
      heading = 270,
      minZ = Config.MediumCarDropLocation.z - 2.0,
      maxZ = Config.MediumCarDropLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-MediumCar:client:target:dropPackage',
...
local function GetRandomPackage()
  packageCoords = Config.MediumCarPickupLocations[math.random(1, #Config.MediumCarPickupLocations)]
...
local function DropPackage()
  ...
  carryPackage = nil
	...
	--Warehouse MediumCar Blip
local function SetLocationBlip()
  -- local RecycleBlip = AddBlipForCoord(Config.MediumCarOutsideLocation.x, Config.MediumCarOutsideLocation.y, Config.MediumCarOutsideLocation.z)
  ...
  AddTextComponentSubstringPlayerName('Medium Car Warehouse')
  EndTextCommandSetBlipName(RecycleBlip)
end

local function buildInteriorDesign()
  for _, pickuploc in pairs(Config.MediumCarPickupLocations) do
    local model = GetHashKey(Config.MediumCarWarehouseObjects[math.random(1, #Config.MediumCarWarehouseObjects)])
    RequestModel(model)
...
local function EnterLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.MediumCarInsideLocation.x, Config.MediumCarInsideLocation.y, Config.MediumCarInsideLocation.z)
  buildInteriorDesign()
  DoScreenFadeIn(500)
...
local function ExitLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.MediumCarOutsideLocation.x, Config.MediumCarOutsideLocation.y, Config.MediumCarOutsideLocation.z + 1)
  DoScreenFadeIn(500)
  ...
  if Config.UseTarget then
    pickupZone = exports['qb-target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
      ...
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-MediumCar:client:target:pickupPackage',
...
RegisterNetEvent('qb-recyclejob-MediumCar:client:target:enterLocation', function()
  EnterLocation()
end)

RegisterNetEvent('qb-recyclejob-MediumCar:client:target:exitLocation', function()
  ExitLocation()
end)

RegisterNetEvent('qb-recyclejob-MediumCar:client:target:toggleDuty', function()
  onDuty = not onDuty
  if onDuty then
...
RegisterNetEvent('qb-recyclejob-MediumCar:client:target:pickupPackage', function()
...
RegisterNetEvent('qb-recyclejob-MediumCar:client:target:dropPackage', function()
  ...
  QBCore.Functions.Progressbar('deliver_reycle_package',  Lang:t("text.unpacking_the_package"), Config.DeliveryActionDuration, false, true, {
      ...
      -- Done
      StopAnimTask(PlayerPedId(), 'mp_car_bomb', 'car_bomb_mechanic', 1.0)
      TriggerServerEvent('qb-MediumCarwarehouse:server:getItemOneThree')
      GetRandomPackage()
  end)
end)
...
CreateThread(function()
  ...
  SetLocationBlip()
  RegisterEntranceMediumCarTarget()
    ...
    while true do
      ...
      if isInsideEntranceZone then
        ...
        if IsControlJustReleased(0, 38) then
          ...
          Wait(500)
          TriggerEvent('qb-recyclejob-MediumCar:client:target:MediumCarenterLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideExitZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-MediumCar:client:target:MediumCarexitLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideDutyZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-MediumCar:client:target:toggleDuty')
          exports['qb-core']:HideText()
        end
      end
      ...
      if onDuty then
        if isInsidePickupZone and not carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-MediumCar:client:target:pickupPackage')
            exports['qb-core']:HideText()
          end
        ...
        if isInsideDeliveryZone and carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-MediumCar:client:target:dropPackage')
            exports['qb-core']:HideText()
          end
        end
      end
